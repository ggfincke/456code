// tests/apps/server/sourceControl/PrTemplateDetection.test.ts
// verifies repository pull request template discovery
...
const PrTemplateDetectionTestLayer = GitVcsDriver.layer.pipe(
  Layer.provide(
    ServerConfig.layerTest(process.cwd(), {
      prefix: '456code-pr-template-test-',
    }),
  ),
...
const runWithTempDirectory = <A, E, R>(
  ...
  Effect.scoped(
    Effect.gen(function* ()
    {
      ...
      const cwd = yield* fileSystem.makeTempDirectoryScoped({ prefix: '456code-pr-template-' })
...
it.effect('reads templates from the requested base tree', () =>
  runWithTempDirectory((cwd) =>
    Effect.gen(function* ()
    {
      ...
      assert.strictEqual(Option.getOrUndefined(yield* detectTemplate(cwd, 'main')), 'base template')
    }),
  ),
)
  ...
  runWithTempDirectory((cwd) =>
    Effect.gen(function* ()
    {
      ...
      const outsideDirectory = yield* fileSystem.makeTempDirectoryScoped({
        prefix: '456code-pr-template-outside-',
      })
