// tests/apps/server/provider/AntigravityAuth.test.ts
// verifies private Antigravity OAuth ownership, leases, cancellation, and redaction
...
} from '../../../../apps/server/src/provider/AntigravityAuth.ts'
import type { AcpSessionRuntimeStartResult } from '../../../../apps/server/src/provider/acp/AcpSessionRuntime.ts'
...
const phase = (
  auth: AntigravityAuth & { readonly flowId: () => string },
  value: ProviderAuthState['phase'],
  sessionId = owner,
) =>
  auth.controller.subscribe(sessionId, auth.flowId()).pipe(
...
)
{
  ...
  let receiveAuthorizationUrl:
    ((url: string) => Effect.Effect<void, AcpErrors.AcpError>) | undefined
  let forwarded = 0
  ...
  return {
    auth: {
      ...
      controller: {
        ...auth.controller,
        start: (...args: Parameters<AntigravityAuth['controller']['start']>) =>
          auth.controller.start(...args).pipe(
            Effect.tap((state) =>
              Effect.sync(() =>
              {
                currentFlowId = state.flowId ?? ''
              }),
            ),
          ),
      },
...
it.layer(NodeServices.layer)('AntigravityAuth', (it) =>
{
  ...
  it.effect('finishes sign-out when the requesting client disconnects', () =>
    Effect.gen(function* ()
    {
      ...
      const result = yield* harness.auth.controller.subscribe(owner, harness.auth.flowId()).pipe(
