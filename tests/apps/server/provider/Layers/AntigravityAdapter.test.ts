...
// verifies official Antigravity ACP lifecycle, permissions, files, and subagents
...
import { deriveNormalizedWorkLogEntries } from '@t3tools/client-runtime/thread-activity'
...
import { ANTIGRAVITY_SIGN_IN_REQUIRED_MESSAGE } from '../../../../../apps/server/src/provider/antigravityAuthSupport.ts'
import type { AcpSessionRuntimeEvent } from '../../../../../apps/server/src/provider/acp/AcpSessionRuntime.ts'
import { makeAntigravityAcpRuntime } from '../../../../../apps/server/src/provider/acp/AntigravityAcpSupport.ts'
import {
  mergeToolCallState,
...
} from '../../../../../apps/server/src/provider/Layers/AntigravityAdapter.ts'

const instanceId = ProviderInstanceId.make('antigravity-test')
const symlinksSupported = HostProcessPlatform.defaultValue() !== 'win32'
const threadId = ThreadId.make('antigravity-thread')
const runtimeSessionBinding = { providerInstanceId: instanceId, threadId, sessionGeneration: 1 }
...
})
{
  ...
  const terminationEvents = yield* Queue.unbounded<AcpErrors.AcpError>()
  const calls: string[] = []
  ...
  const emitNative = (
    event:
      ...
      | { readonly _tag: 'ConnectionTerminated'; readonly error: AcpErrors.AcpError },
  ) =>
    event._tag === 'ConnectionTerminated'
      ? Queue.offer(terminationEvents, event.error).pipe(Effect.asVoid)
  ...
  const runtime: Effect.Success<ReturnType<AntigravityAdapterOptions['makeRuntime']>> = {
    ...
    getTerminationEvents: () => Stream.fromQueue(terminationEvents),
    drainEvents,
    prompt: (payload, promptOptions) =>
      Effect.gen(function* ()
      {
        ...
        if (promptOptions !== undefined) yield* promptOptions
...
it.layer(layer)('AntigravityAdapter', (it) =>
{
  ...
  it.effect('waits for native cancellation before a steer changes the model', () =>
    Effect.gen(function* ()
    {
      ...
      expect(initialPrompt.content).toEqual([{ type: 'text', text: 'First prompt' }])
      ...
      expect(replacement.content).toEqual([{ type: 'text', text: 'Steer the turn' }])
  ...
  it.effect('keeps a launched batch active while child tools continue', () =>
    Effect.gen(function* ()
    {
      ...
      yield* h.waitForEvent((event) => event.type === 'turn.completed')
      expect(h.seen.filter((event) => event.type === 'task.completed')).toHaveLength(0)
      const idleBatch = h.seen.find((event) => event.type === 'task.updated')!
      expect(idleBatch.payload).toMatchObject({
        taskId: started.toolCall.toolCallId,
        taskType: 'subagent_batch',
        status: 'idle',
      ...
      const entries = deriveNormalizedWorkLogEntries(runtimeEventToActivities(idleBatch), {
  ...
  it.effect.skipIf(!symlinksSupported)(
    'rejects client file reads and writes through symlinks outside the session roots',
    () =>
      Effect.gen(function* ()
      {
        const fs = yield* FileSystem.FileSystem
        const path = yield* Path.Path
        const h = yield* makeHarness()
        const cwd = yield* fs.makeTempDirectoryScoped({ prefix: 't3-agy-fs-symlink-' })
        const outside = yield* fs.makeTempDirectoryScoped({ prefix: 't3-agy-outside-symlink-' })
        const secret = path.join(outside, 'secret.txt')
        yield* fs.writeFileString(secret, 'outside')
        yield* fs.symlink(secret, path.join(cwd, 'secret-link'))
        ...
        const escapedRead = yield* read({
          sessionId: nativeSessionId,
          path: path.join(cwd, 'secret-link'),
        }).pipe(Effect.flip)
        expect(escapedRead._tag).toBe('AcpRequestError')

        const escapedWrite = yield* write({
          sessionId: nativeSessionId,
          path: path.join(cwd, 'directory-link', 'missing', 'new.txt'),
          content: 'outside',
        }).pipe(Effect.flip)
        expect(escapedWrite._tag).toBe('AcpRequestError')
        expect(yield* fs.exists(path.join(outside, 'missing', 'new.txt'))).toBe(false)
      }).pipe(Effect.scoped),
  )
