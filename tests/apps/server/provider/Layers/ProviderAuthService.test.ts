// tests/apps/server/provider/Layers/ProviderAuthService.test.ts
// verifies provider authentication routing, session fencing, and private subscriptions
...
import type { ProviderAuthController } from '../../../../../apps/server/src/provider/Services/ProviderAuthService.ts'
...
} from '../../../../../apps/server/src/provider/Services/ProviderSessionDirectory.ts'
import { makeProviderAuthService } from '../../../../../apps/server/src/provider/Layers/ProviderAuthService.ts'
...
}): ProviderInstance
{
  return {
    ...
    resolveContinuationIdentity: Effect.succeed({ driverKind, continuationKey: input.instanceId }),
    get snapshot(): never
    {
...
)
{
    ...
    Effect.provide(
      Layer.mergeAll(
        ...
        Layer.mock(ProviderService)({
          ...
          stopSession: ({ threadId, expectedProviderInstanceId }) =>
            Effect.suspend(() =>
            {
              assert.isTrue(gateClosed)
              stopRequests.push({
  ...
  return { service, actions, sessions, bindings, stopRequests }
})
...
describe('ProviderAuthService', () =>
{
  ...
  it.effect("keeps sign-in state private and accepts the owner's redirect URL", () =>
    Effect.gen(function* ()
    {
      ...
      const ownerStates = yield* service
        .subscribe({ instanceId, flowId }, owner)
        .pipe(Stream.take(1), Stream.runCollect)
      const otherStates = yield* service
        .subscribe({ instanceId, flowId }, otherOwner)
        .pipe(Stream.take(1), Stream.runCollect)
  ...
  it.effect.each([
    ...
    ({ initialEnabled, closeBeforeReplacement }) =>
      Effect.gen(function* ()
      {
        ...
        const firstClient = yield* observeAuth(service.subscribe({ instanceId, flowId }, owner))
        const secondClient = yield* observeAuth(
          service.subscribe({ instanceId, flowId }, otherOwner),
        )
  ...
  it.effect('does not miss a replacement during the first instance lookup', () =>
    Effect.gen(function* ()
    {
      ...
      const states = yield* service.subscribe({ instanceId, flowId }, owner).pipe(
  ...
  it.effect('ends the subscription with a setup error when the instance is removed', () =>
    Effect.gen(function* ()
    {
      ...
      const { states, fiber } = yield* observeAuth(service.subscribe({ instanceId, flowId }, owner))
  ...
  it.effect.each([
    ...
    Effect.gen(function* ()
    {
      ...
      const operations: Effect.Effect<
        ProviderAuthState | ProviderAuthLogoutResult | ReadonlyArray<ProviderAuthState>,
        ProviderSetupError
      >[] = [
        ...
        Stream.runCollect(service.subscribe({ instanceId: id, flowId }, owner)),
      ]
  ...
  it.effect('stops active and persisted sessions once before native logout', () =>
    Effect.gen(function* ()
    {
      const { service, actions, sessions, bindings, stopRequests } = yield* makeHarness({
        bindings: [
      ...
      assert.deepStrictEqual(
        stopRequests.map((request) => request.expectedProviderInstanceId),
  ...
  it.effect('does not log out when stopping a session fails', () =>
    Effect.gen(function* ()
    {
      const { service, actions, sessions } = yield* makeHarness({
        sessions: [makeSession('active')],
  ...
  it.effect('returns native logout failures to the caller', () =>
    Effect.gen(function* ()
    {
