...
} from '../../../../../apps/server/src/provider/Layers/CursorProvider.ts'
import {
  discoverCursorSkills,
  hasCursorSkillMention,
  probeCursorSkills,
  rewriteCursorSkillMentions,
} from '../../../../../apps/server/src/provider/Drivers/CursorSkills.ts'
...
describe('Cursor skills', () =>
{
  it('discovers recursive project skills with project precedence', async () =>
    await runNode(
      Effect.gen(function* ()
      {
        ...
        const realUserHome = yield* fileSystem.realPath(userHome)
        ...
        expect(skills).toEqual([
          ...
          },
          {
            name: 'machine-global',
            description: 'user-global skill',
            path: path.join(realUserHome, '.cursor', 'skills', 'machine-global', 'SKILL.md'),
            scope: 'user',
            enabled: true,
          },
          {
  ...
  it('detects and invokes digit-leading Cursor skills without rewriting money', () =>
  {
    ...
    // repeated checks must not carry a global-regex cursor
...
)
{
  ...
  {
    const content = yield* fileSystem.readFileString(filePath).pipe(Effect.catch(() => Effect.void))
...
const cursorCliCommandMissingMessage = [
  ...
  `Install or enable the Cursor CLI, make sure \`${missingCursorBinaryPath}\` is on PATH, then restart 456code.`,
  'See https://cursor.com/docs/cli/installation.',
].join(' ')
