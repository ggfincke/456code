...
// verifies official Antigravity account catalogs and workspace snapshot retention
...
} satisfies AcpSessionRuntimeStartResult['sessionSetupResult']
...
const testLayer = ServerSettingsService.layerTest()
...
{
  it.effect('does not probe while disabled', () =>
    Effect.scoped(
      Effect.gen(function* ()
      {
        ...
        expect(snapshot).toMatchObject({
          ...
          capabilities: { conversationRollback: 'unsupported' },
        })
  ...
  it.effect('bounds workspace metadata without starting sessions for workspace lookup', () =>
    Effect.scoped(
      Effect.gen(function* ()
      {
        ...
        expect(snapshot.workspaceSnapshots).toHaveLength(16)
        expect(snapshot.workspaceSnapshots?.[0]?.cwd).toBe('/workspace-19')
        expect(snapshot.slashCommands).toEqual(commands)
        expect(yield* Ref.get(harness.probeCalls)).toBe(1)
      }),
    ),
