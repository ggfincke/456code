...
} from '../../../../../apps/server/src/provider/Layers/ProviderRegistry.ts'
...
function resolveManualMaintenance(provider: ProviderDriverKind)
{
  return () =>
    Effect.succeed(
...
)('ProviderRegistry', (it) =>
{
  ...
  describe('ProviderRegistryLive', () =>
  {
    ...
    it('stores only the newest sixteen workspace snapshots and refreshes an existing cwd', () =>
    {
      const provider = {
        instanceId: ProviderInstanceId.make('codex'),
        driver: ProviderDriverKind.make('codex'),
      ...
      let result = provider as ServerProvider
      for (let index = 0; index < 17; index += 1)
      {
        result = upsertProviderWorkspaceSnapshot(result, `/workspace/${index}`, {
          ...provider,
          checkedAt: `2026-09-08T00:${String(index).padStart(2, '0')}:00.000Z`,
          skills: [
            {
              name: `project-${index}`,
              path: `/workspace/${index}/SKILL.md`,
              enabled: true,
            },
          ],
        })
      ...
      assert.strictEqual(result.workspaceSnapshots?.length, 16)
      assert.strictEqual(result.workspaceSnapshots?.[0]?.cwd, '/workspace/1')
      assert.strictEqual(result.workspaceSnapshots?.at(-1)?.cwd, '/workspace/16')
      assert.deepStrictEqual(result.skills, provider.skills)

      const refreshed = upsertProviderWorkspaceSnapshot(result, '/workspace/5', {
        ...provider,
        checkedAt: '2026-09-08T01:00:00.000Z',
        skills: [{ name: 'refreshed', path: '/workspace/5/SKILL.md', enabled: true }],
      })
      assert.strictEqual(refreshed.workspaceSnapshots?.length, 16)
      assert.strictEqual(refreshed.workspaceSnapshots?.at(-1)?.cwd, '/workspace/5')
      assert.strictEqual(refreshed.workspaceSnapshots?.at(-1)?.skills[0]?.name, 'refreshed')
    })

    it.effect(
      'deduplicates workspace probes, refreshes cached entries, retains failures, and fences rebuilds',
      () =>
        Effect.gen(function* ()
        {
          ...
          const firstWorkspace = {
            ...
            skills: [{ name: 'first', path: '/workspace/SKILL.md', enabled: true }],
          } satisfies ServerProvider
          const refreshedWorkspace = {
            ...machineProvider,
            checkedAt: '2026-09-08T00:02:00.000Z',
            skills: [{ name: 'refreshed', path: '/workspace/SKILL.md', enabled: true }],
          } satisfies ServerProvider
          ...
          const rebuiltProvider = {
            ...machineProvider,
            checkedAt: '2026-09-08T00:04:00.000Z',
          } satisfies ServerProvider
          const probeCalls = yield* Ref.make(0)
          const probeMode = yield* Ref.make<
            'first' | 'failure' | 'refreshed' | 'timeout' | 'stale'
          >('first')
          ...
          const firstInstance = makeInstance(
            machineProvider,
            Effect.fn('testWorkspaceSnapshotForCwd')(function* ()
            {
              yield* Ref.update(probeCalls, (count) => count + 1)
              switch (yield* Ref.get(probeMode))
              {
                case 'first':
          ...
          const rebuiltInstance = makeInstance(rebuiltProvider, () =>
            Effect.succeed(refreshedWorkspace),
          )
          ...
          yield* Effect.gen(function* ()
          {
            ...
            const expectedWorkspaceCheckedAt = DateTime.formatIso(yield* DateTime.now)
            ...
            const duplicateRefresh = yield* registry
              ...
              .pipe(Effect.forkChild)
            yield* Effect.yieldNow
            assert.strictEqual(yield* Ref.get(probeCalls), 1)
            yield* Deferred.succeed(releaseFirst, undefined)
            yield* Fiber.join(firstRefresh)
            yield* Fiber.join(duplicateRefresh)
            assert.strictEqual(
              (yield* registry.getProviders)[0]?.workspaceSnapshots?.[0]?.skills[0]?.name,
              'first',
            )
            assert.strictEqual(
              (yield* registry.getProviders)[0]?.workspaceSnapshots?.[0]?.checkedAt,
              expectedWorkspaceCheckedAt,
            )
            ...
            assert.strictEqual(
              (yield* readProviderStatusCache(filePath))?.workspaceSnapshots,
              undefined,
            )

            yield* Ref.set(probeMode, 'failure')
            yield* registry.refreshWorkspaceSnapshot({ instanceId, cwd: '/workspace' })
            assert.strictEqual(yield* Ref.get(probeCalls), 2)
            assert.strictEqual(
              (yield* registry.getProviders)[0]?.workspaceSnapshots?.[0]?.skills[0]?.name,
              'first',
            )

            yield* Ref.set(probeMode, 'refreshed')
            yield* registry.refreshWorkspaceSnapshot({ instanceId, cwd: '/workspace' })
            assert.strictEqual(yield* Ref.get(probeCalls), 3)
            assert.strictEqual(
              (yield* registry.getProviders)[0]?.workspaceSnapshots?.[0]?.skills[0]?.name,
              'refreshed',
            )

            yield* Ref.set(probeMode, 'timeout')
            ...
            assert.strictEqual(yield* Ref.get(probeCalls), 4)
            assert.strictEqual(
              (yield* registry.getProviders)[0]?.workspaceSnapshots?.[0]?.skills[0]?.name,
              'refreshed',
            )

            yield* Ref.set(probeMode, 'stale')
            const staleRefresh = yield* registry
              .refreshWorkspaceSnapshot({ instanceId, cwd: '/workspace' })
              .pipe(Effect.forkChild)
            ...
            for (
              let attempt = 0;
              attempt < 50 && providers[0]?.checkedAt !== rebuiltProvider.checkedAt;
              attempt += 1
            )
            {
              yield* Effect.yieldNow
            ...
            assert.strictEqual(providers[0]?.checkedAt, rebuiltProvider.checkedAt)
            assert.strictEqual(providers[0]?.workspaceSnapshots, undefined)
    ...
    it('replaces successful Codex inventories while retaining the last failed catalog', () =>
    {
      ...
      {
        assert.deepStrictEqual(mergeProviderSnapshot(previousProvider, clearedProvider).models, [])
      }
    })
    ...
    it.effect('does not run provider probes during layer construction', () =>
      Effect.gen(function* ()
      {
        ...
        const maintenanceResolutions = yield* Ref.make<
          ReadonlyArray<{ readonly fresh?: boolean } | undefined>
        >([])
        ...
        yield* Effect.gen(function* ()
        {
          ...
          assert.deepStrictEqual(yield* Ref.get(maintenanceResolutions), [{ fresh: true }])
        }).pipe(Effect.provide(runtimeServices))
      }),
    )
    ...
    it.effect('restamps provider snapshots when a live continuation source changes', () =>
      Effect.gen(function* ()
      {
        ...
        {
          const registry = yield* ProviderRegistry.ProviderRegistry
          assert.equal((yield* registry.getProviders)[0]?.continuation?.groupKey, 'codex:file:v1:a')

          yield* Ref.set(continuationKey, 'codex:file:v1:b')

          assert.equal((yield* registry.getProviders)[0]?.continuation?.groupKey, 'codex:file:v1:b')
        }).pipe(Effect.provide(runtimeServices))
      }),
    )
    ...
    it.effect('runs an instance model refresh hook before reading its new snapshot', () =>
      Effect.gen(function* ()
      {
        const driverKind = ProviderDriverKind.make('refresh-hook')
        const instanceId = ProviderInstanceId.make('refresh-hook')
        const initialProvider = {
          instanceId,
          driver: driverKind,
          status: 'warning',
        ...
        const snapshot = yield* Ref.make<ServerProvider>(initialProvider)
        const refreshModelsCount = yield* Ref.make(0)
        const instance = {
          instanceId,
          driverKind,
          continuationIdentity: {
            driverKind,
            continuationKey: 'refresh-hook:instance:refresh-hook',
          },
          resolveContinuationIdentity: Effect.succeed({
            driverKind,
            continuationKey: 'refresh-hook:instance:refresh-hook',
          ...
          snapshot: {
            applyUsageLimits: () => Effect.void,
            invalidateUsageLimits: () => Effect.void,
            resolveMaintenance: resolveManualMaintenance(driverKind),
            getSnapshot: Ref.get(snapshot),
            refresh: Effect.die(new Error('snapshot.refresh must not bypass refreshModels')),
            streamChanges: Stream.empty,
          },
          refreshModels: Ref.update(refreshModelsCount, (count) => count + 1).pipe(
            Effect.andThen(Ref.set(snapshot, refreshedProvider)),
          ),
          adapter: {} as ProviderInstance['adapter'],
          textGeneration: {} as ProviderInstance['textGeneration'],
        } satisfies ProviderInstance
        ...
        const runtimeServices = yield* Layer.build(
          ProviderRegistryLive.pipe(
            ...
            Layer.provideMerge(
              ServerConfig.layerTest(process.cwd(), {
                prefix: 't3-provider-registry-refresh-hook-',
              }),
            ),
            Layer.provideMerge(NodeServices.layer),
          ),
        ...
        yield* Effect.gen(function* ()
        {
          ...
          assert.equal(yield* Ref.get(refreshModelsCount), 1)
          assert.deepStrictEqual(refreshed[0]?.models, refreshedProvider.models)
          assert.equal(refreshed[0]?.checkedAt, refreshedProvider.checkedAt)
        }).pipe(Effect.provide(runtimeServices))
      }).pipe(Effect.scoped),
    )
    ...
    it.effect('returns the cached provider list when a manual refresh fails', () =>
      Effect.gen(function* ()
      {
        ...
        yield* Effect.gen(function* ()
        {
          ...
          assert.deepStrictEqual(yield* registry.refreshInstance(codexInstanceId), [cachedProvider])
        }).pipe(Effect.provide(runtimeServices))
      }),
    )
    ...
    it.effect('propagates real Codex probe failures to the aggregator at boot', () =>
      Effect.gen(function* ()
      {
        ...
        const runtimeServices = yield* Layer.build(providerRegistryLayer).pipe(Scope.provide(scope))
    ...
    it.effect('keeps cursor disabled and skips probing when the provider setting is disabled', () =>
      Effect.gen(function* ()
      {
        ...
        yield* Effect.gen(function* ()
        {
          ...
          assert.strictEqual(cursorProvider?.message, 'Cursor is disabled in 456code settings.')
    ...
    it.effect('skips codex probes entirely when the provider is disabled', () =>
      Effect.gen(function* ()
      {
        ...
        assert.strictEqual(status.installed, false)
        assert.strictEqual(status.message, 'Codex is disabled in 456code settings.')
      }),
    )
  })
  ...
  describe('checkClaudeProviderStatus', () =>
  {
    it.effect('returns ready when claude is installed and authenticated', () =>
      Effect.gen(function* ()
      {
        const status = yield* checkClaudeProviderStatus(defaultClaudeSettings, claudeCapabilities())
        assert.strictEqual(status.status, 'ready')
        assert.strictEqual(status.installed, true)
        assert.strictEqual(status.auth.status, 'authenticated')
      }).pipe(
    ...
    it.effect.each<
      Readonly<{
        ...
        slug: string
        expected:
          | { readonly kind: 'include'; readonly modelName: string }
          | {
              readonly kind: 'hide'
              readonly message: string
            }
          | {
              readonly kind: 'includeWithDefaultEffort'
              readonly defaultEffort: { id: string; label: string; isDefault: true }
            }
      }>
    >([
      {
        name: 'includes Claude Opus 5 on supported Claude Code versions',
        version: '2.1.219',
        slug: 'claude-opus-5',
        expected: { kind: 'include', modelName: 'Claude Opus 5' },
      },
      {
        name: 'hides Claude Opus 5 on older Claude Code versions',
        version: '2.1.218',
        slug: 'claude-opus-5',
        expected: {
          kind: 'hide',
      ...
      {
        name: 'includes Claude Fable 5 on supported Claude Code versions',
        version: '2.1.169',
        slug: 'claude-fable-5',
        expected: { kind: 'include', modelName: 'Claude Fable 5' },
      },
      {
        name: 'hides Claude Fable 5 on older Claude Code versions',
        version: '2.1.168',
        slug: 'claude-fable-5',
        expected: {
          kind: 'hide',
      ...
      {
        name: 'includes Claude Opus 4.7 with xhigh as the default effort on supported versions',
        version: '2.1.111',
        slug: 'claude-opus-4-7',
        expected: {
          kind: 'includeWithDefaultEffort',
          defaultEffort: { id: 'xhigh', label: 'Extra High', isDefault: true },
        },
      },
      {
        name: 'hides Claude Opus 4.7 on older Claude Code versions',
        version: '2.1.110',
        slug: 'claude-opus-4-7',
        expected: {
          kind: 'hide',
    ...
    ])('$name', ({ version, slug, expected }) =>
      Effect.gen(function* ()
      {
        const status = yield* checkClaudeProviderStatus(defaultClaudeSettings, claudeCapabilities())
        if (expected.kind === 'include')
        {
          const model = status.models.find((entry) => entry.slug === slug)
          assert.strictEqual(model?.name, expected.modelName)
          return
        }
        if (expected.kind === 'hide')
        {
          assert.strictEqual(
            status.models.some((model) => model.slug === slug),
            false,
          )
          assert.strictEqual(status.message, expected.message)
          return
        }
        const opus47 = status.models.find((model) => model.slug === slug)
        if (!opus47)
        {
          assert.fail(`Expected ${slug} to be present for Claude Code v${version}.`)
        }
        if (!opus47.capabilities)
        {
          assert.fail(`Expected ${slug} capabilities to be present for Claude Code v${version}.`)
        }
    ...
    it.effect('returns warning when the Claude initialization result is unavailable', () =>
      Effect.gen(function* ()
      {
        const status = yield* checkClaudeProviderStatus(defaultClaudeSettings, noClaudeCapabilities)
