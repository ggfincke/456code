// tests/apps/server/provider/Drivers/AntigravitySkills.test.ts
// verifies bounded native Antigravity skill discovery and precedence
...
} from '../../../../../apps/server/src/provider/Drivers/AntigravitySkills.ts'
...
it.layer(NodeServices.layer)('discoverAntigravitySkills', (it) =>
{
  ...
  it.effect('loads only user roots for the machine fallback catalog', () =>
    Effect.gen(function* ()
    {
      const path = yield* Path.Path
      const input = yield* makeWorkspace()
      yield* writeSkill(
        path.join(input.userHome, '.gemini', 'config', 'skills', 'global'),
        '---\nname: global\n---\n',
      )
      yield* writeSkill(
        path.join(input.cwd, '.gemini', 'skills', 'project'),
        '---\nname: project\n---\n',
      )

      assert.deepEqual(
        (yield* discoverAntigravityUserSkills(input.userHome)).map((skill) => skill.name),
        ['global'],
      )
    }),
  )
