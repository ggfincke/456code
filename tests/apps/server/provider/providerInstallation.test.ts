// tests/apps/server/provider/providerInstallation.test.ts
// verifies provider installation routing and custom-runtime refusal
...
import { makeProviderInstallation } from '../../../../apps/server/src/provider/providerInstallation.ts'
...
function instance(kind = driver): ProviderInstance
{
  return {
    ...
    resolveContinuationIdentity: Effect.succeed({ driverKind: kind, continuationKey: instanceId }),
    get adapter(): never
    {
...
describe('provider installation routing', () =>
{
  ...
  it.effect('keeps external installs manual without hiding shared install status', () =>
    Effect.gen(function* ()
    {
      const harness = yield* makeHarness({
        settings: {
          providers: {
            antigravity: { officialRuntime: { mode: 'custom', executablePath: '/external/acp' } },
          },
        },
      })
  ...
  it.effect(
    "removes only the managed runtime without resolving another instance's legacy binary",
    () =>
      Effect.gen(function* ()
      {
        ...
        const harness = yield* makeHarness({
          settings: {
            providerInstances: {
              [other]: {
                driver,
                config: { binaryPath: '/external/legacy-agy' },
              },
            },
          },
        })
