...
// covers git-backed version control behavior

// @effect-diagnostics nodeBuiltinImport:off - setting an old mtime forces Git to run the clean filter
...
} from '../../../../apps/server/src/vcs/GitVcsDriverCore.ts'
...
it.layer(TestLayer)('GitVcsDriver core integration', (it) =>
{
  ...
  describe('review diff previews', () =>
  {
    ...
    it.effect('keeps a/ and b/ prefixes when repository config disables them', () =>
      Effect.gen(function* ()
      {
        const cwd = yield* makeTmpDir()
        const { initialBranch } = yield* initRepoWithCommit(cwd)
  ...
  describe('refName operations', () =>
  {
    ...
    it.effect('creates, checks out, renames, and lists refs', () =>
      Effect.gen(function* ()
      {
        ...
        const refs = yield* driver.listRefs({ cwd })
        assert.equal(refs.refs.find((refName) => refName.name === 'feature/renamed')?.current, true)
      }),
    )
  ...
  describe('worktree operations', () =>
  {
    ...
    it.effect('prunes stale registrations when removing an already-gone worktree', () =>
      Effect.gen(function* ()
      {
        ...
        assert.notInclude(yield* git(cwd, ['worktree', 'list', '--porcelain']), 'stale')
      }),
    )
  ...
  describe('remote operations', () =>
  {
    ...
    it.effect('allows pushes to run longer than the default command timeout', () =>
      Effect.gen(function* ()
      {
        ...
        const pushing = yield* driver
          .pushCurrentBranch(cwd, null, undefined)
          .pipe(Effect.forkChild({ startImmediately: true }))
    ...
    it.effect('releases an unlimited push process when the caller cancels', () =>
      Effect.gen(function* ()
      {
        const delegate = yield* ChildProcessSpawner.ChildProcessSpawner
        const pushStarted = yield* Deferred.make<void>()
        const pushKilled = yield* Deferred.make<void>()
        const pushHandle = ChildProcessSpawner.makeHandle({
          pid: ChildProcessSpawner.ProcessId(1),
          exitCode: Effect.never,
          isRunning: Effect.succeed(true),
          kill: () => Deferred.succeed(pushKilled, undefined).pipe(Effect.asVoid),
          unref: Effect.succeed(Effect.void),
          stdin: Sink.drain,
          stdout: Stream.empty,
          stderr: Stream.empty,
        ...
        const cancellablePushSpawner = ChildProcessSpawner.make((command) =>
        {
          if (ChildProcess.isStandardCommand(command) && command.args[0] === 'push')
          {
            return Effect.acquireRelease(
              Deferred.succeed(pushStarted, undefined).pipe(Effect.as(pushHandle)),
              () => pushHandle.kill().pipe(Effect.ignore),
            )
          }
          return delegate.spawn(command)
        })
        const driver = yield* makeGitVcsDriverCore().pipe(
          Effect.provideService(ChildProcessSpawner.ChildProcessSpawner, cancellablePushSpawner),
          Effect.provide(ServerConfigLayer),
        )
        const cwd = yield* makeTmpDir()
        const remote = yield* makeTmpDir('git-remote-')
        ...
        assert.isTrue(yield* Deferred.isDone(pushKilled))
      }),
    )
    ...
    it.effect('still pushes a git-mangled tracking alias to its upstream head', () =>
      Effect.gen(function* ()
      {
        ...
        assert.equal(yield* git(cwd, ['rev-parse', '--abbrev-ref', 'HEAD']), 'upstream/effect-atom')
