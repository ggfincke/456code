// tests/apps/server/diagnostics/ProcessDiagnostics.test.ts
// verify process diagnostics behavior
  ...
  it.effect('does not allow signaling the diagnostics query process', () =>
    Effect.gen(function* ()
    {
      ...
      expect(result).toEqual({
        ...
        signal: 'SIGINT',
        signaled: false,
        message: Option.some('Process 4242 is not a live descendant of the 456code server.'),
      })
    }),
  )
})
