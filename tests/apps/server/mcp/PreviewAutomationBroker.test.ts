...
it.effect.each([
  ...
  Effect.scoped(
    Effect.gen(function* ()
    {
      ...
      expect(error).toMatchObject({ _tag: tag, threadId: scope.threadId })
      expect(error.cause).toBe(remoteError)
...
it.effect('pins a provider session to its initial host despite later focus changes', () =>
  Effect.scoped(
    Effect.gen(function* ()
    {
      ...
      expect(yield* broker.invoke<string>({ scope, operation: 'status', input: {} })).toBe('second')
      yield* broker.focusHost({
        clientId: 'client-first',
        environmentId: scope.environmentId,
...
it.effect('does not move a live legacy assignment to another runtime for resize', () =>
  Effect.scoped(
    Effect.gen(function* ()
    {
      ...
      expect(yield* broker.invoke<string>({ scope, operation: 'status', input: {} })).toBe('legacy')
