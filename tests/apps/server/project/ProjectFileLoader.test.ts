...
const writeProjectFile = Effect.fn('writeProjectFile')(function* (cwd: string, contents: string)
{
  ...
  yield* fileSystem.writeFileString(path.join(cwd, '456code.json'), contents).pipe(Effect.orDie)
})
...
it.layer(TestLayer)('ProjectFileLoader', (it) =>
{
  describe('load', () =>
  {
    it.effect('loads and decodes a valid 456code.json', () =>
      Effect.gen(function* ()
      {
    ...
    it.effect('returns none when 456code.json is missing', () =>
      Effect.gen(function* ()
      {
        const loader = yield* ProjectFileLoader.ProjectFileLoader
        const cwd = yield* makeTempDir
