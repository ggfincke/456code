...
it.layer(GitManagerTestLayer)('GitManager', (it) =>
{
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* ()
      {
        ...
        yield* runGit(repoDir, ['checkout', '-b', '456code/pr-488/statemachine'])
        ...
        const status = yield* manager.status({ cwd: repoDir })
        expect(status.refName).toBe('456code/pr-488/statemachine')
        expect(status.pr).toEqual({
          number: 488,
          title: 'Rebase this PR on latest main',
          url: 'https://github.com/pingdotgg/codething-mvp/pull/488',
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* ()
      {
        ...
        yield* runGit(repoDir, ['remote', 'add', 'fork-seed', forkDir])
        yield* runGit(repoDir, ['push', '-u', 'fork-seed', 'statemachine'])
        yield* runGit(repoDir, ['checkout', '-b', '456code/pr-142/statemachine'])
        yield* runGit(repoDir, ['branch', '--set-upstream-to', 'fork-seed/statemachine'])
        yield* configureVisibleRemoteUrlWithLocalRewrite(
          repoDir,
          'fork-seed',
        ...
        const { manager, ghCalls } = yield* makeManager({
          ghScenario: {
            prListByHeadSelector: {
              // @effect-diagnostics-next-line preferSchemaOverJson:off
              '456code/pr-142/statemachine': JSON.stringify([]),
              // @effect-diagnostics-next-line preferSchemaOverJson:off
  ...
  it.effect('generates PR content against the remote base when the local base is stale', () =>
    Effect.gen(function* ()
    {
      ...
      yield* runGit(repoDir, ['fetch', 'origin'])
      yield* runGit(repoDir, ['checkout', '--no-track', '-b', 'feature/remote-base', 'origin/main'])
      NodeFS.writeFileSync(NodePath.join(repoDir, 'feature.txt'), 'feature\n')
  ...
  it.effect('creates cross-repo PRs with the fork owner selector and default base branch', () =>
    Effect.gen(function* ()
    {
      ...
      yield* runGit(repoDir, ['commit', '-m', 'Feature commit'])
      yield* runGit(repoDir, ['push', '-u', 'fork-seed', 'statemachine'])
      yield* runGit(repoDir, ['checkout', '-b', '456code/pr-91/statemachine'])
      yield* runGit(repoDir, ['branch', '--set-upstream-to', 'fork-seed/statemachine'])
      yield* configureVisibleRemoteUrlWithLocalRewrite(
        repoDir,
        'fork-seed',
  ...
  it.effect('preserves both branch materialization failures when the fallback also fails', () =>
    Effect.gen(function* ()
    {
      ...
      expect(error).toMatchObject({
        ...
        localBranch: '456code/pr-93/feature/missing-fork-branch',
      })
  ...
  it.effect('reuses an existing dedicated worktree for the PR head branch', () =>
    Effect.gen(function* ()
    {
      ...
      yield* runGit(repoDir, ['checkout', 'main'])
      const worktreePath = NodePath.join(repoDir, '..', `pr-existing-${NodePath.basename(repoDir)}`)
  ...
  it.effect(
    'prepares fork PR worktrees without blocking on or overwriting a root main collision',
    () =>
      Effect.gen(function* ()
      {
