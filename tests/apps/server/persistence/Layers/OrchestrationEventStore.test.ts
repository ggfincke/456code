// tests/apps/server/persistence/Layers/OrchestrationEventStore.test.ts
// verify orchestration event store behavior
...
import { PersistenceDecodeError } from '../../../../../apps/server/src/persistence/Errors.ts'
...
layer('OrchestrationEventStore', (it) =>
{
  ...
  it.effect('reads one aggregate through the captured head across pruned global gaps', () =>
    Effect.gen(function* ()
    {
      ...
      // the same stream id in a different aggregate and unrelated invalid
      // rows must never reach this thread's decoder
      yield* sql`
        INSERT INTO orchestration_events (
          ...
          'unrelated-invalid', 'thread', 'unrelated-invalid-thread', 0,
          'thread.activity-appended', '2026-01-01T00:00:00.000Z', 'server', '{', '{'
        )
      `
  ...
  it.effect('bounds aggregate stats and counts utf-8 bytes without decoding payloads', () =>
    Effect.gen(function* ()
    {
      const store = yield* OrchestrationEventStore
      const sql = yield* SqlClient.SqlClient
  ...
  it.effect('keeps later aggregate pages below the captured head', () =>
    Effect.gen(function* ()
    {
      const store = yield* OrchestrationEventStore
