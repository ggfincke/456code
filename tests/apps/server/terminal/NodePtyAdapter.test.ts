// tests/apps/server/terminal/NodePtyAdapter.test.ts
// verify node pty adapter behavior
...
import * as NodePtyAdapter from '../../../../apps/server/src/terminal/NodePtyAdapter.ts'
...
it.effect('preserves a caller-provided TERM case-insensitively on win32', () =>
  Effect.gen(function* ()
  {
    spawn.mockClear()
    const adapter = yield* PtyAdapter.PtyAdapter
