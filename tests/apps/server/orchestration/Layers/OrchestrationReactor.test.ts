...
describe('OrchestrationReactor', () =>
{
  ...
  effectIt.effect(
    'fences provider admission and drains both inbox lanes before downstream owners',
    () =>
    {
      const started: string[] = []
      const drained: string[] = []
  ...
  effectIt.effect(
    'catches a dirty startup fence before provider and checkpoint domain work resumes',
    () =>
    {
  ...
  effectIt.effect(
    'fails shutdown without certifying downstream drain when an inbox lane is blocked',
    () =>
    {
      const started: string[] = []
      const drained: string[] = []
      ...
      {
        const reactor = yield* OrchestrationReactor
        const exit = yield* Effect.exit(reactor.shutdown)
