...
it.layer(NodeServices.layer)('decider project scripts', (it) =>
{
  ...
  ])
  {
    it.effect(`rejects a new script ID that cannot have a shortcut: ${id}`, () =>
      Effect.gen(function* ()
      {
        ...
        expect(failure.message).toContain(String(MAX_SCRIPT_ID_LENGTH))
  ...
  it.effect('accepts a script ID at the shortcut length limit', () =>
    ...
    {
      const readModel = yield* projectWithScripts([])
      const scripts = [script('a'.repeat(MAX_SCRIPT_ID_LENGTH))]
      const result = yield* decideOrchestrationCommand({
        readModel,
        command: {
          type: 'project.meta.update',
  ...
  it.effect('keeps legacy scripts editable and removable while allowing valid additions', () =>
    Effect.gen(function* ()
    {
