...
}): string
{
  ...
  NodeFS.writeFileSync(sourcePath, withWorkspaceCwd(fixture(input.fixtureName), input.workspaceDir))
  return sourcePath
}
  ...
  Effect.acquireUseRelease(
    ...
    (harness) =>
      Effect.gen(function* ()
      {
        ...
        const secondPath = NodePath.join(secondDirectory, `rollout-second-${nativeSessionId}.jsonl`)
