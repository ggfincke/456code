...
{
  const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), 't3-cli-project-lookup-state-'))
...
it.layer(NodeServices.layer)('project lookup with unavailable workspaces', (it) =>
{
  ...
  it.effect.each([true, false])(
    ...
    (removeWorkspace) =>
      Effect.gen(function* ()
      {
        ...
        assert.isNull(retained.projects.find((candidate) => candidate.id === project.id)!.deletedAt)
        assert.isNull(
  ...
  it.effect(
    'renames by ID and normalized stored path, then force removes after the directory is gone',
    () =>
      Effect.gen(function* ()
      {
        const { baseDir, workspaceRoot, project } = yield* makeProjectLookupFixture(true, true)
        const normalizedMissingInput = `${workspaceRoot}${NodePath.sep}.`
        yield* runCliWithRuntime([
          'project',
          'rename',
          project.id,
