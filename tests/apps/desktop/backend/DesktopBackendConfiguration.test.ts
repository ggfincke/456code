...
describe('DesktopBackendConfiguration', () =>
{
  ...
  it('resolvePrimaryLabel is runSync-safe against the real WSL availability probe', async () =>
  {
    ...
    // oxlint-disable-next-line 456code/no-manual-effect-runtime-in-tests -- This test intentionally replicates the sync IPC handler's runSync path to catch a regression to async-only resolution; it.effect would mask it.
    ...
    try
    {
      ...
      // oxlint-disable-next-line 456code/no-manual-effect-runtime-in-tests -- Same reason: this is the synchronous resolution the IPC handler performs.
