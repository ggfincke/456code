...
vi.mock('electron', async (importOriginal) => ({
  ...
  session: {
    fromPartition: vi.fn(() => ({
      getUserAgent: vi.fn(() => 'Mozilla/5.0 Electron/41.5.0 456code/1.2.3'),
...
} from '../../../../apps/desktop/src/ipc/channels.ts'
  ...
  it('recognizes only same-origin renderer navigations', () =>
  {
    assert.isTrue(
      DesktopWindow.isSameOriginRendererNavigation({
        applicationUrl: 'code456://app/',
        navigationUrl: 'code456://app/settings/connections',
      }),
    )
    assert.isFalse(
      DesktopWindow.isSameOriginRendererNavigation({
  ...
  it.effect('recovers when the development renderer is temporarily unreachable', () =>
    Effect.gen(function* ()
    {
      ...
      yield* Effect.gen(function* ()
      {
        ...
        didFailLoad({}, -9, 'ERR_UNEXPECTED', 'code456-dev://app/', true)
  ...
  it('retries only transient failures for the development renderer', () =>
  {
    assert.isTrue(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
        applicationUrl: 'code456-dev://app/',
        errorCode: -102,
        isMainFrame: true,
        validatedUrl: 'code456-dev://app/',
      }),
    )
    assert.isFalse(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
