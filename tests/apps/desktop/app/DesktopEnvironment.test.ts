// tests/apps/desktop/app/DesktopEnvironment.test.ts
// verify desktop environment behavior
...
describe('DesktopEnvironment', () =>
{
  it.effect('derives state paths and development identity inside Effect', () =>
    Effect.gen(function* ()
    {
      ...
      assert.equal(environment.linuxWmClass, '456code-dev')
      assert.deepEqual(
  ...
  it.effect('keeps implicit development state separate from production state', () =>
    Effect.gen(function* ()
    {
      ...
      assert.equal(development.stateDir, '/Users/alice/.456code/dev')
      assert.equal(production.stateDir, '/Users/alice/.456code/userdata')
    }),
  )
  ...
  it.effect('uses a configured app user model id override', () =>
    Effect.gen(function* ()
    {
      const environment = yield* makeEnvironment(
        {},
        {
          T3CODE_DESKTOP_APP_USER_MODEL_ID: ' com.ggfincke.456code.dev.local ',
          VITE_DEV_SERVER_URL: 'http://localhost:5173',
        },
      )

      assert.equal(environment.appUserModelId, 'com.ggfincke.456code.dev.local')
    }),
  )
