...
import { MobilePreferencesStore } from '../../../../../apps/mobile/src/persistence/mobile-preferences'
...
describe('mobile cloud link environment client', () =>
{
  ...
  it.effect('decodes relay environment list responses before returning records', () =>
    Effect.gen(function* ()
    {
      ...
      const records = yield* withCloudServices(listCloudEnvironments({ clerkToken: 'clerk-token' }))
  ...
  it.effect('reuses the status-and-connect token when connecting from the cloud list', () =>
    Effect.gen(function* ()
    {
      ...
      expect(environmentTokenBody.get('client_label')).toBe('456code Mobile')
