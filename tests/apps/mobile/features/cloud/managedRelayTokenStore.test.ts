...
} from '../../../../../apps/mobile/src/features/cloud/managedRelayTokenStore'
...
it.effect('falls back to an empty cache when persisted data is invalid', () =>
  ...
  {
    secureStore.clear()
    secureStore.set('code456.cloud.relay-access-tokens', 'not-json')
...
it.effect('logs structured storage failures before falling back to an empty cache', () =>
{
  ...
  return Effect.gen(function* ()
  {
    ...
    expect(context.cause).toMatchObject({
      operation: 'read',
      storageKey: 'code456.cloud.relay-access-tokens',
      cause,
    })
