...
import {
  ...
  cryptoLayer,
} from '../../../../../apps/mobile/src/features/cloud/dpop'
...
describe('mobile DPoP', () =>
{
  ...
  it.effect('rejects malformed persisted proof keys', () =>
    Effect.gen(function* ()
    {
      secureStore.set('code456.cloud.dpop-proof-key', `{"kty":"EC","crv":"P-256","d":42}`)
  ...
  it.effect('signs DPoP proofs with RFC 9449 htu normalization', () =>
    Effect.gen(function* ()
    {
      ...
      expect(proofHtu(proof.proof)).toBe('https://relay.example.test/v1/environments/env-1/connect')
      expect(
        verifyDpopProof({
          proof: proof.proof,
          method: 'POST',
