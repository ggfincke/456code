...
} from '../../../../apps/mobile/src/persistence/imperative'
import { toStableSavedRemoteConnection } from '../../../../apps/mobile/src/lib/connection'
...
describe('mobile connection storage', () =>
{
  ...
  it('preserves secure-storage read failures with operation and key context', async () =>
  {
    ...
    await expect(loadSavedConnections()).rejects.toMatchObject({
      ...
      operation: 'read',
      key: 'code456.connections',
      cause,
      message: 'Mobile secure storage operation read failed for key code456.connections.',
    })
  })

  it('logs structured decode failures before using the empty fallback', async () =>
  {
    await mocks.setItemAsync('code456.connections', '{')
    ...
    expect(warn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        message: 'Failed to decode mobile storage value for key code456.connections.',
      }),
    )
  ...
  it('loads legacy preferences when SQLite is unavailable', async () =>
  {
    ...
    await mocks.setItemAsync('code456.preferences', JSON.stringify({ baseFontSize: 17 }))
  ...
  it('falls back to secure storage when SQLite cannot save preferences', async () =>
  {
    ...
    const fallback = JSON.parse(mocks.getStoredValue('code456.preferences.fallback') ?? '') as {
  ...
  it('reconciles fallback preferences after SQLite recovers', async () =>
  {
    ...
    expect(mocks.getStoredValue('code456.preferences.fallback')).toBeNull()
  })
  ...
  {
    mocks.setPreferencesJson(JSON.stringify({ baseFontSize: 21 }), 30)
    await mocks.setItemAsync('code456.preferences', JSON.stringify({ baseFontSize: 19 }))
