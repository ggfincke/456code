...
import { installEnvironmentHttpTest } from "../../../apps/web/test/environmentHttpTest";
import {
...
function installTestBrowser(url: string) {
  ...
  vi.stubGlobal("document", { title: "456code" });
...
describe("resolveInitialServerAuthGateState", () => {
  ...
  afterEach(async () => {
    ...
    const { __resetServerAuthBootstrapForTests } =
      await import("../../../apps/web/src/environments/primary");
  ...
  it("takes a pairing token from the location hash and strips it immediately", async () => {
    ...
    const { takePairingTokenFromUrl } = await import("../../../apps/web/src/environments/primary");
  ...
  it("rejects a blank pairing token with a structured validation error", async () => {
    const { PrimaryEnvironmentPairingCredentialRequiredError, submitServerAuthCredential } =
      await import("../../../apps/web/src/environments/primary/auth");
