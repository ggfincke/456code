// tests/apps/web/bundledDev.test.ts
// verify React refresh ordering and Tailwind reloads in bundled development
...
import { tailwindPlugins } from '../../../apps/web/vite/tailwind'

const execFile = NodeUtil.promisify(NodeChildProcess.execFile)
const webFile = (path: string) => new URL(`../../../apps/web/${path}`, import.meta.url)
...
it('initializes React refresh before a shared UI chunk runs in bundled dev', async () =>
{
  ...
  try
  {
    ...
    {
      await NodeFSP.copyFile(webFile(file), NodePath.join(root, file))
    }
...
export const startup = Promise.resolve().then(() => globalThis.onStarted(Shared()));`,
    ...
    server = await createServer({
      configFile: false,
      root,
      publicDir: NodeURL.fileURLToPath(webFile('public')),
      logLevel: 'silent',
      resolve: {
      ...
      build: {
        rolldownOptions: {
          ...
          output: {
            // reproduce lazy-route shared chunks without needing a browser to trigger compilation
            codeSplitting: {
              groups: [
    ...
    // run generated modules so their real import order and refresh checks execute
...
it('hot updates Tailwind classes when a source file changes in bundled dev', async () =>
{
  ...
  try
  {
    ...
    server = await createServer({
      ...
      resolve: {
        alias: {
          tailwindcss: NodeURL.fileURLToPath(webFile('node_modules/tailwindcss/index.css')),
        },
      },
      experimental: { bundledDev: true },
      plugins: [
        ...tailwindPlugins(true),
        {
          ...
          {
            // wait until the socket can receive the bundled development ready message
            await connected
          },
        },
      ],
    ...
    socket.addEventListener('message', ({ data }) =>
    {
      ...
      {
        // generated bundles are available over http only after this notification
        if (
