    ...
    Effect.gen(function* ()
    {
      ...
      const transport = yield* CodexProtocol.makeCodexAppServerPatchedProtocol({
        ...
        onNotification: (notification) =>
          ...
          }).pipe(Effect.andThen(Deferred.succeed(notificationReceived, undefined)), Effect.asVoid),
      })
  ...
  it.effect.each([1, 7, 1024])(
    ...
    (chunkSize) =>
      Effect.gen(function* ()
      {
        ...
        const notifications: Array<CodexProtocol.CodexAppServerIncomingNotification> = []
        const notificationsReceived = yield* Deferred.make<void>()
              ...
              Effect.andThen(
                Effect.suspend(() =>
                  notifications.length === 3
                    ? Deferred.succeed(notificationsReceived, undefined)
                    : Effect.void,
                ),
              ),
              Effect.asVoid,
        ...
        yield* Deferred.await(notificationsReceived)
