...
)
{
  override get message(): string
  {
    ...
    return `456code window is unavailable during ${this.stage} for SSH authentication to ${this.destination} (request: ${request}).`
  }
}
...
export type DesktopSshPasswordPromptResolveError =
  DesktopSshPromptInvalidRequestIdError | DesktopSshPromptExpiredError

export type DesktopSshPasswordPromptError =
  DesktopSshPasswordPromptRequestError | DesktopSshPasswordPromptResolveError
