...
// successful browser document or navigation redirect. Positive and negative
// results are cached briefly by candidate URL and listener identity.
//
...
export class PortDiscovery extends Context.Service<
  PortDiscovery,
  {
    ...
    ) => Effect.Effect<ReadonlyArray<DiscoveredLocalServer>, never, Scope.Scope>
...
>()('456code/preview/PortScanner/PortDiscovery')
{}
...
{
  if (raw.length === 0 || raw.trim().length !== raw.length) return null
  try
  {
...
export const make = Effect.gen(function* PortDiscoveryMake()
{
  ...
  const pollTick = Effect.fn('PortDiscovery.pollTick')(
    function* ()
    {
      // keep candidate selection and subscriber projection in one scan epoch.
      const notifications = yield* scanSemaphore.withPermits(1)(
        Effect.gen(function* ()
        {
          const stateBeforeScan = yield* Ref.get(stateRef)
          if (stateBeforeScan.retainCount <= 0)
          {
            return [] as Array<readonly [Listener, ReadonlyArray<DiscoveredLocalServer>]>
          }
          const configuredUrls = [
            ...new Set(
              [...stateBeforeScan.listeners.values()].flatMap(
  ...
  const retain: PortDiscovery['Service']['retain'] = Effect.acquireRelease(
    Ref.update(stateRef, (state) => ({ ...state, retainCount: state.retainCount + 1 })),
    () =>
  ...
  const subscribe: PortDiscovery['Service']['subscribe'] = Effect.fn('PortDiscovery.subscribe')(
    (input, listener) =>
      Effect.gen(function* ()
      {
        const configuredUrls = normalizeConfiguredUrls(input.configuredUrls)
        // keep readiness probes interruptible; only listener ownership is scoped.
        return yield* scanSemaphore.withPermits(1)(
          Effect.gen(function* ()
          {
            const snapshot = yield* scanUnlocked(configuredUrls)
            const initialSnapshot = projectWebProbeSnapshot(snapshot, configuredUrls)
            yield* Effect.acquireRelease(
              Ref.update(stateRef, (state) =>
              {
                const listeners = new Map(state.listeners)
                listeners.set(listener, { configuredUrls, lastSnapshot: initialSnapshot })
