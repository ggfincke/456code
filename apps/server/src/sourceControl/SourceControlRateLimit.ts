...
// coordinates bounded provider rate-limit pauses
...
export interface RateLimitLease extends RateLimitKey
{
...
>()('456code/sourceControl/SourceControlRateLimit')
{}
...
export function retryAtFromHeader(value: string | undefined, now: number): number | undefined
{
  ...
  {
    const retryAt = now + Number(normalized) * 1_000
  ...
  )(function* (input)
  {
    ...
    yield* Ref.update(entries, (current) =>
    {
      ...
      if (previous !== undefined && previous.generation > input.lease)
      {
        ...
        const retryAt = Math.max(previous.retryAt, input.retryAt ?? previous.retryAt)
