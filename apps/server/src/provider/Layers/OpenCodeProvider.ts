...
export function openCodeSkillsToServerProviderSkills(
  input: OpenCodeInventory['skills'],
        ...
        probe: {
          ...
          message:
            openCodeSettings.serverUrl.trim().length > 0
              ? 'OpenCode is disabled in 456code settings. A server URL is configured.'
              : 'OpenCode is disabled in 456code settings.',
        },
      })
    }
...
): Effect.fn.Return<ServerProviderDraft, never, OpenCodeRuntime | OpenCodeServerOwner>
{
  ...
  if (!isExternalServer)
  {
    ...
    if (!version)
    {
      return fallback(
        new Error(
          `Unable to determine OpenCode version from \`opencode --version\` output. 456code requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
        ),
        null,
      )
    }
