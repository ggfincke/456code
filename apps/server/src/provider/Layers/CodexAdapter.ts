...
): ReadonlyArray<ProviderRuntimeEvent>
{
  ...
  {
    const payload = readPayload(EffectCodexSchema.V2ThreadRealtimeClosedNotification, event.payload)
    return [
      {
    ...
    Effect.scoped(
      Effect.gen(function* ()
      {
        ...
        const runtimeInput: CodexSessionRuntimeOptions = {
          ...
          ...(mcpSession
            ? {
                browserToolsAvailable: mcpSession.previewToolsAvailable,
                environment: {
                  ...(options?.environment ?? process.env),
                  CODE456_MCP_BEARER_TOKEN: mcpSession.authorizationHeader.replace(
                    /^Bearer\s+/,
                    '',
                  ),
                },
                appServerArgs: [
                  '-c',
                  `mcp_servers.code456.url=${mcpSession.endpoint}`,
                  '-c',
