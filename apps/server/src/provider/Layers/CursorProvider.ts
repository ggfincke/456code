    ...
    if (!cursorSettings.enabled)
    {
      return buildCursorServerProvider({
        ...
        probe: {
          ...
          status: 'warning',
          auth: { status: 'unknown' },
          message: 'Cursor is disabled in 456code settings.',
        },
      })
    }
  ...
  Effect.gen(function* ()
  {
    ...
    const acpContext = yield* Layer.build(
      AcpSessionRuntime.layer({
        ...
        cwd: process.cwd(),
        clientInfo: { name: 'code456-provider-probe', version: '0.0.0' },
...
function buildCursorCliCommandMissingMessage(binaryPath: string): string
{
  return [
    ...
    `Install or enable the Cursor CLI, make sure \`${binaryPath}\` is on PATH, then restart 456code.`,
    `See ${CURSOR_CLI_INSTALLATION_DOCS_URL}.`,
  ].join(' ')
}
