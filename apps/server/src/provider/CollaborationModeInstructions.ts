...
3. When the orchestrate MCP toolkit is available, persist the resolved plan with \`orchestrate_plan_upsert\`, then ALSO emit the fenced \`orchestrate-plan\` JSON block with the same \`runId\` — the fence is the timeline anchor the client renders the persisted revision into. Without the toolkit, the fenced block alone is the supported form. Include a stable \`runId\`, workflow, task, unique stage ids, provider/model/effort, read or edit mode, worker counts, scalar scopes, \`totalWorkers\`, and \`maxWorkers\`.
