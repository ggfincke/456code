...
// share one lazy local OpenCode server across a provider instance's borrowers
...
>()('456code/provider/OpenCodeServerOwner')
{}
...
})
{
  ...
  {
    if (expected !== undefined && state.server !== expected) return
    const scope = state.serverScope
    state.server = null
    state.serverScope = null
    if (scope !== null) yield* Scope.close(scope, Exit.void).pipe(Effect.ignore)
  })
  ...
  const acquireServer = mutex.withPermit(
    Effect.gen(function* ()
    {
      ...
      // publish or close the fresh scope atomically; startup remains interruptible
  ...
  const releaseServer = (server: OpenCodeRuntime.OpenCodeServerProcess) =>
    mutex.withPermit(
      Effect.gen(function* ()
      {
        // an older borrower must not release a replacement server's lease
        if (state.server !== server) return
        ...
        state.idleCloseFiber = yield* Effect.sleep(OPENCODE_SERVER_IDLE_TTL).pipe(
          Effect.andThen(
            mutex.withPermit(
              Effect.gen(function* ()
              {
                if (state.server !== server || state.borrowers > 0) return
                state.idleCloseFiber = null
                yield* closeServer(server)
              }),
            ),
