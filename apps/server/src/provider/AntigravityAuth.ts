...
// coordinates private personal Google OAuth flows and Antigravity process leases
...
): Effect.fn.Return<AntigravityAuth, never, Crypto.Crypto | Scope.Scope>
{
  ...
  const controller: ProviderAuthController = {
    ...
    subscribe: (ownerSessionId, flowId) =>
      SubscriptionRef.changes(snapshot).pipe(
        Stream.map((value) =>
          value.ownerSessionId === ownerSessionId && value.state.flowId === flowId
            ? value.state
