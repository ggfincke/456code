  ...
  if (serverConfig.autoBootstrapProjectFromCwd)
  {
    yield* Effect.gen(function* ()
    {
      ...
      if (Option.isNone(existingProject))
      {
        ...
        nextProjectDefaultModelSelection = fallbackModelSelection
      ...
      else
      {
        ...
        nextProjectDefaultModelSelection =
          existingProject.value.defaultModelSelection ?? fallbackModelSelection
      }
...
// threads left claiming an active provider session after a restart would hang
// forever waiting for events nobody will send; settle them to a terminal state
export const reconcileProviderSessions = Effect.gen(function* ()
{
  ...
  for (const thread of orphanedThreads)
  {
    ...
    // binding cleanup is best effort; the projection settle below is what
    // unblocks the user
    yield* Effect.gen(function* ()
    {
      ...
      else
      {
        ...
        if (serverConfig.mode !== 'desktop')
        {
          yield* Effect.logInfo(
            'Authentication required. Open 456code using the pairing URL.',
