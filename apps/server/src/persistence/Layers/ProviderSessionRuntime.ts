...
// persists provider session runtime metadata and resume cursors
...
export const make = Effect.gen(function* ()
{
  ...
  const list: ProviderSessionRuntimeRepository['Service']['list'] = () =>
    listRuntimeRows(undefined).pipe(
      ...
      Effect.flatMap((rows) =>
        // skip stale rows so one old provider record cannot disable enumeration
...
export const layer = ProviderSessionRuntimeRepositoryLive
