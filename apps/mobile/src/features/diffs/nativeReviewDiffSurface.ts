...
const NATIVE_REVIEW_DIFF_MODULE_NAME = 'Code456ReviewDiffSurface'
...
export interface NativeReviewDiffRow
{
  ...
  readonly changeType?:
    'modified' | 'new' | 'deleted' | 'renamed' | 'rename-pure' | 'rename-changed'
...
export function isPendingNativeViewRegistration(error: unknown): boolean
{
  ...
  return (
    ...
    (error.message.includes('Unable to find the class') &&
      error.message.includes('Code456ReviewDiffView view with tag'))
  )
}
