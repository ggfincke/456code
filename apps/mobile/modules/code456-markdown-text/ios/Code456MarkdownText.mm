...
static void Code456MarkdownTextApplyParagraphStyles(
...
static void Code456MarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
...
@protocol Code456MarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface Code456MarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<Code456MarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<Code456MarkdownOutsideTapTarget>)target;

@end

static const void *Code456MarkdownOutsideTapCoordinatorKey =
    &Code456MarkdownOutsideTapCoordinatorKey;

@implementation Code456MarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<Code456MarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<Code456MarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, Code456MarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<Code456MarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static Code456MarkdownOutsideTapCoordinator *
Code456MarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  Code456MarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, Code456MarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[Code456MarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        Code456MarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface Code456MarkdownText () <RCTCode456MarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface Code456MarkdownText () <Code456MarkdownOutsideTapTarget>
@end

@implementation Code456MarkdownText {
  UIView * _view;
  UITextView * _textView;
  Code456MarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<Code456MarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const Code456MarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    Code456MarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, Code456MarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [Code456MarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<Code456MarkdownTextProps const>(_props);
  ...
  Code456MarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  Code456MarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::Code456MarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::Code456MarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<Code456MarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<Code456MarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<Code456MarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == Code456MarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == Code456MarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == Code456MarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == Code456MarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const Code456MarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (Code456MarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[Code456MarkdownTextRun class]]) {
      continue;
    }

    Code456MarkdownTextRun* textChild = (Code456MarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::Code456MarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::Code456MarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> Code456MarkdownTextCls(void)
{
  return Code456MarkdownText.class;
}
