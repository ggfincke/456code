...
public final class Code456KeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("Code456KeyboardCommands")

    View(Code456KeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: Code456KeyboardCommandsView, commands: [String]) in
...
public final class Code456KeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.code456FirstResponder == nil else { return }
...
private extension UIView {
  var code456FirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.code456FirstResponder { return responder }
    }
    return nil
  }
}
