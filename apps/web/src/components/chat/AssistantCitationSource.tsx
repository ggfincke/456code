// apps/web/src/components/chat/AssistantCitationSource.tsx
// reveal and highlight citation sources across virtual timeline row lifecycles
...
// keep source text marked while its comment editor is open without changing selection
...
}): () => void
{
  ...
  {
    if (!stopped) validate(records.some((record) => source.contains(record.target)))
  })
  const dispose = () =>
  {
    if (stopped) return
