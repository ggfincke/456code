...
// prepare cited timeline rows for source highlighting and guarded navigation

import type { LegendListRef } from '@legendapp/list/react'
import type { TurnId } from '@t3tools/contracts'
import { useCallback, useEffect, useRef, useState, type RefObject } from 'react'
import type { TimelineEntry } from '../../session-logic'
import type { MessagesTimelineRow } from './messages-timeline/MessagesTimeline.logic'
...
})
{
  ...
  useEffect(() =>
  {
    ...
    if (navigationRef.current?.target.key !== request.key)
    {
      ...
      navigationRef.current = { target, requestedPages: new Set(), done: false }
      setReady(null)
      onManualNavigation()
    }
    if (!viewport || historyLoading) return
    const navigation = navigationRef.current
    if (!navigation || navigation.done || navigation.target.activationRef.current.dismissed) return
