...
// render durable citation links and editable composer citation chips
...
})
{
  ...
  const sourceLink = (className: string) => (
    ...
    >
      <QuoteIcon aria-hidden className={COMPOSER_INLINE_CHIP_ICON_CLASS_NAME} />
  ...
  return (
    ...
    >
      {onRemove ? (
        sourceLink(
          'inline-flex h-full min-w-0 items-center gap-[0.33em] rounded-sm text-inherit no-underline focus-visible:outline-2 focus-visible:outline-primary',
        )
      ) : (
        <Tooltip>
          <TooltipTrigger
            render={sourceLink(
              'inline-flex h-full min-w-0 items-center gap-[0.33em] rounded-sm text-inherit no-underline hover:bg-primary/10 focus-visible:outline-2 focus-visible:outline-primary',
            )}
          />
      ...
      {commentEditor ? (
        <Popover open={commentEditor.open} onOpenChange={commentEditor.onOpenChange}>
          ...
          >
            <PencilIcon aria-hidden className="size-[0.85em]" />
          </PopoverTrigger>
          {commentEditor.open ? (
            <PopoverPopup
            ...
            >
              <AssistantCitationCommentEditor
                ...
                onCancel={() =>
                  {
                  if (commentEditor.onCancel) commentEditor.onCancel()
                  else commentEditor.onOpenChange(false)
                }}
              />
            </PopoverPopup>
          ) : null}
      ...
      {onRemove ? (
        ...
        >
          <XIcon aria-hidden className="size-[0.85em]" />
        </button>
      ) : null}
    </span>
  )
