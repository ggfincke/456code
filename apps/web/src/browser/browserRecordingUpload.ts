...
// transfer a completed browser recording to its requesting environment
...
// only the finished encoded file crosses the environment connection
...
): Promise<string>
{
  ...
  const result = await runAttachmentUploadCycle({
    ...
    resolveHttpBaseUrl: () => readPreparedConnection(environmentId)?.httpBaseUrl ?? null,
    transport: (url) =>
    {
  ...
  throw new PreviewAutomationRecordingTransferError({ threadId, cause })
}
