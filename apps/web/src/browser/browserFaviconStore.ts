// apps/web/src/browser/browserFaviconStore.ts
// persist captured favicons by physical project and canonical origin
...
import { normalizeHostname } from './browserTargetResolver'

const BROWSER_FAVICON_STORAGE_KEY = '456code:browser-favicons:v1'
...
export interface BrowserFaviconStoreState
{
  ...
  // capture buffering only; never persisted.
...
const pendingOriginKey = (pageUrl: string): string | null => faviconKey('pending', pageUrl, null)

const projectMatchesThreadEnvironment = (
  threadRef: ScopedThreadRef,
  projectRef: ScopedProjectRef,
): boolean => threadRef.environmentId === projectRef.environmentId

const addPendingFavicon = (
  pendingByThreadKey: Record<string, PendingFaviconsByOrigin>,
  threadKey: string,
  ...
  persist(
    (set) => ({
      ...
      recordFavicon: (key, dataUrl, capturedAt, aliases = []) =>
        set((state) =>
        {
          if (!isStorableFaviconDataUrl(dataUrl) || !isValidFaviconCapturedAt(capturedAt))
          {
            return state
          }
...
{
  if (!projectMatchesThreadEnvironment(threadRef, projectRef)) return
  ...
  const hostname =
    environmentHostname !== undefined
      ...
      : canCanonicalizeFaviconWithoutEnvironment(favicon.pageUrl)
        ? null
        : undefined
  const matchingProjectRef =
    projectRef && projectMatchesThreadEnvironment(threadRef, projectRef) ? projectRef : null
  if (
    matchingProjectRef &&
    hostname !== undefined &&
    recordFaviconForProject(matchingProjectRef, favicon, hostname)
  )
  {
    return true
  }
...
{
  if (!projectMatchesThreadEnvironment(threadRef, projectRef)) return false
  const threadKey = scopedThreadKey(threadRef)
...
{
  const key = projectRef ? faviconKey(scopedProjectKey(projectRef), url, environmentHostname) : null
  if (!key) return null
