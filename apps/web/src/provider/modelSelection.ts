// apps/web/src/provider/modelSelection.ts
...
import { getComposerProviderState } from '../lib/composerProviderState'
import { type ModelEsque } from '../lib/modelDisplay'
import { getDefaultServerModel, getProviderModels, resolveSelectableProvider } from './models'
import { type ProviderInstanceEntry, deriveProviderInstanceEntries } from './instances'
...
): AppModelOption[]
{
  ...
  // missing display options must not override a hidden raw catalog entry
...
function toAppModelOption(model: ServerProvider['models'][number]): AppModelOption
{
  ...
  if (model.isLegacy === true && !model.isCustom) option.isLegacy = true
  return option
}
