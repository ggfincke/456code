// apps/web/src/lib/selectionActions.ts
// coordinate delayed pointer and keyboard actions for stable text selections
...
// open selection actions after release while leaving multi-click gestures time to finish
...
})
{
  ...
  return {
    ...
    selectionChanged()
    {
      ...
      if (pointerDown || gestureActive) onDismiss('cancel')
      else if (!dismissed && timer === null && frame === null) schedule(0)
    },
    dispose()
    {
      cancel()
