...
// own bounded attachment uploads and draft upload references
...
async function runUpload(job: UploadJob): Promise<void>
{
  ...
  const result = await runAttachmentUploadCycle({
    ...
    resolveHttpBaseUrl: () =>
    {
      ...
      return connection?.httpBaseUrl ?? null
    },
    transport: (url) =>
      uploadBytes({
        url,
...
export function startAttachmentUpload(input: {
  ...
  readonly draftTarget?: ComposerThreadTarget
  readonly verifyReady?: boolean
  readonly environmentReady?: boolean
}): void
{
  // hydrated images need a new mint; cached capabilities do not imply an RPC session exists yet
  if (input.environmentReady === false) return
  ...
  const returningUpload = previous?.environmentId === input.environmentId ? previous : undefined
  ...
  const job: UploadJob = {
    ...
    ...(previous ? { previous } : {}),
    ...(returningUpload
      ? { persistedAttachmentId: returningUpload.attachmentId }
...
}): void
{
  ...
  startAttachmentUpload({ ...input, verifyReady: true })
}
...
// an acknowledged turn owns these bytes now; forget queue state without deleting the server copy
export function acceptAttachmentUploads(imageIds: ReadonlyArray<string>): void
{
  for (const imageId of imageIds)
  {
    if (readAttachmentUpload(imageId)?.status === 'ready') clearUploadState(imageId)
  }
}
...
}): ChatKnownAttachment[] | null
{
  const attachments: ChatKnownAttachment[] = []
