...
import { RelayEnvironmentConnectScope, RelayEnvironmentStatusScope } from '@t3tools/contracts/relay'
      ...
      return yield* Effect.tryPromise({
        try: () => readCachedClerkToken(nowMillis),
          ...
          new ManagedRelaySessionError({
            message: 'Could not obtain the relay session token.',
            cause,
          }),
      })
    }),
  ...
  return session.readClerkToken().pipe(
    Effect.flatMap((token) =>
      token
        ...
        : Effect.fail(
            new ManagedRelaySessionError({
              message: 'The relay session token is unavailable.',
            }),
          ),
    ),
  )
...
): Effect.Effect<string, ManagedRelaySessionError>
{
  ...
  if (!session || session.accountId !== accountId)
  {
    return Effect.fail(
      new ManagedRelaySessionError({
        message: 'Sign in to the relay before loading relay data.',
      }),
    )
  }
...
): ManagedRelaySnapshotState<A>
{
  ...
  if (result._tag === 'Failure')
  {
    ...
    error = cause instanceof Error ? cause.message : 'Could not load relay data.'
