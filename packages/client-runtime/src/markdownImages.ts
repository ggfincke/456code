...
// classify markdown image sources for authenticated client loading
...
function parseFileUrl(value: string): string | null
{
  try
  {
    ...
    return pathname.length === 0 ? null : normalizeWindowsDrivePath(pathname)
  }
  catch
  {
    return null
...
{
  const windows = WINDOWS_DRIVE_PATH_PATTERN.test(workspaceRoot) || workspaceRoot.startsWith('\\\\')
...
// keep filesystem sources on their environment host instead of exposing them to image views
...
): MarkdownImageSource
{
  ...
  if (DIRECT_IMAGE_SOURCE_PATTERN.test(source)) return { _tag: 'Direct', uri: source }
