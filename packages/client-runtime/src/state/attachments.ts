...
// coordinate authenticated attachment upload and verification cycles
...
// signed upload targets cannot redirect local file bytes outside the selected environment
export function resolveAttachmentUploadUrl(
  httpBaseUrl: string,
  relativeUrl: string,
): string | null
{
...
export async function runAttachmentUploadCycle<E, RE>(input: {
  ...
  readonly resolveHttpBaseUrl: () => string | null
...
{
  const httpBaseUrl = input.resolveHttpBaseUrl()
  if (!httpBaseUrl)
  {
    return {
      status: 'failed',
  ...
  const url =
    input.resolveHttpBaseUrl() === httpBaseUrl
      ? resolveAttachmentUploadUrl(httpBaseUrl, minted.value.relativeUrl)
      : null
  if (!url)
  {
    return {
