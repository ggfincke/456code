...
// builds environment-scoped reactive runtime state
...
export function createAtomCommandScheduler(): AtomCommandScheduler
{
  ...
  return {
    ...
    ): Promise<AtomCommandResult<A, E>> =>
    {
      ...
      {
        const existing = state.singleFlight.get(key) as Promise<AtomCommandResult<A, E>> | undefined
        if (existing !== undefined)
        {
          return existing
        }
...
>
{
  return EnvironmentRegistry.pipe(Effect.flatMap((registry) => registry.run(environmentId, effect)))
}
