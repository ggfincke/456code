  ...
  )
  {
    ...
    if (!releaseAsset)
    {
      return yield* new RelayClientInstallError({
        ...
        message: `456code does not provide a managed relay client binary for ${platform}-${arch}.`,
      })
    }
    ...
    yield* fileSystem
      ...
      .pipe(wrapInstallFailure('write_failed', 'Could not create the relay client tool directory.'))
