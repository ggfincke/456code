...
// classify hosts for private-network and public-provider boundaries
...
const isSpecialPurposeIpv4Address = (parts: readonly number[]): boolean =>
  ...
  parts[0]! >= 224 ||
  // suppress the whole protocol-assignment block. IANA marks .9 and .10
  // globally reachable, but privacy-safe false negatives are preferable.
...
// only globally routable hosts may be disclosed to a public favicon provider.
export const isPublicFaviconHost = (host: string): boolean =>
{
  // one trailing dot is a valid absolute DNS name. Repeated dots can conceal
  // legacy numeric forms such as 127.1.
