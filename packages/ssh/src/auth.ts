...
const SSH_ASKPASS_DIR_NAME = '456code-ssh-askpass'
...
# Invoked by ssh via SSH_ASKPASS when 456code re-runs ssh with a cached password
...
printf '456code ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when 456code re-runs\r
...
[Console]::Error.WriteLine("456code ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`
...
>
{
  ...
  return {
    ...
    ...(platform === 'win32' || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: '456code' }),
  }
})
