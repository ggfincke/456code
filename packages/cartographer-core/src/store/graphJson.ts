import { isAnalysisCoverage } from "../contracts/analysisCoverage.js";
import type {
  CartographerGraph,
  GraphEdge,
  GraphJourney,
...
function parseGraphRule(value: unknown): GraphRule | undefined {
  ...
  if (raw.mandatory !== undefined && typeof raw.mandatory !== "boolean") return undefined;
  const hasSystem = raw.fromSystem !== undefined || raw.toSystem !== undefined;
  if (
    hasSystem &&
    (raw.generated !== true ||
      typeof raw.fromSystem !== "string" ||
      typeof raw.toSystem !== "string")
  )
    return undefined;
  ...
  return {
    ...
    ...(typeof raw.mandatory === "boolean" ? { mandatory: raw.mandatory } : {}),
    ...(typeof raw.fromSystem === "string" ? { fromSystem: raw.fromSystem } : {}),
    ...(typeof raw.toSystem === "string" ? { toSystem: raw.toSystem } : {}),
  };
}
...
export function normalizeGraphJson(graph: CartographerGraph): CartographerGraph {
  if (graph.coverage !== undefined && !isAnalysisCoverage(graph.coverage)) {
    throw new Error("invalid graph analysis coverage");
  }
