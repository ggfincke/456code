...
function validEdge(value: unknown): boolean {
  return (
    ...
    (value.namespaceImports === undefined ||
      (Array.isArray(value.namespaceImports) &&
        value.namespaceImports.every(
          (entry) =>
            isObject(entry) && isNonEmptyString(entry.name) && typeof entry.typeOnly === "boolean",
        ))) &&
...
function assertRequiredShape(
  ...
  const missingArrays = ["nodes", "edges", "groups"].filter(
