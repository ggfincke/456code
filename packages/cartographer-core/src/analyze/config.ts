...
function parseEnforcement(value: unknown, where: string): RuleEnforcement | undefined {
  ...
  if (
    ...
    throw new Error(
      `invalid ${CONFIG_FILE}: ${where} "enforcedBy.line" must be a positive integer`,
    );
  }
...
function parseDependencyRule(rule: unknown, index: number): AuthoredGraphRule {
  ...
  if (raw.mandatory !== undefined && typeof raw.mandatory !== "boolean") {
    throw new Error(`invalid ${CONFIG_FILE}: ${where} "mandatory" must be a boolean`);
  }
...
function parseJourney(journey: unknown, index: number): AuthoredJourney {
  ...
  if (raw.stops.length > MAX_JOURNEY_STOPS) {
    throw new Error(
      `invalid ${CONFIG_FILE}: ${where} "stops" exceeds ${MAX_JOURNEY_STOPS} entries`,
    );
  }
