...
import { buildAtlasIndex } from "../store/atlasIndex/build.ts";
import { graphContentDigest } from "../store/atlasIndex/digest.ts";
import { parseAtlasIndex } from "../contracts/atlasIndexCodec.ts";
import { buildGraph } from "../analyze/graph.ts";
import { buildSymbolTable, exportList } from "../analyze/symbols.ts";
import { computeCoChanges } from "../analyze/cochange.ts";
import {
  loadConfig,
  MAX_JOURNEYS,
  MAX_RUNTIMES,
...
import { parsePositiveInt } from "../cli/lib/args.ts";
import { normalizeGraphJson } from "../store/graphJson.ts";
import { trackedTempRoot } from "../test-support/trackedTempRoot.ts";
import { runBuild } from "../cli/commands/build.ts";
import { loadContextQuery } from "../query/contextQuery.ts";
...
describe("package-aware import analysis", () => {
  it("uses each importing package config, resolves exports and persists bounded missing-local evidence", async () => {
    const root = gitRepo();
    const files: Record<string, string> = {
      "package.json": JSON.stringify({ private: true, workspaces: ["apps/*", "packages/*"] }),
      "apps/one/package.json": JSON.stringify({ name: "@example/one", type: "module" }),
      "apps/two/package.json": JSON.stringify({ name: "@example/two", type: "module" }),
      "apps/one/tsconfig.json": JSON.stringify({
        compilerOptions: { paths: { "@/*": ["./src/*"] } },
      }),
      "apps/two/tsconfig.json": JSON.stringify({
        compilerOptions: { baseUrl: ".", paths: { "@/*": ["./lib/*"] } },
      }),
      "apps/one/src/value.ts": "export const value = 1\n",
      "apps/two/lib/value.ts": "export const value = 2\n",
      "apps/one/src/index.ts":
        "import { value } from '@/value'\nimport { shared } from '@example/shared/value'\nvoid [value, shared]\n" +
        Array.from({ length: 22 }, (_, index) => `import './missing-${index}.js'`).join("\n") +
        "\nimport 'third-party-uninstalled'\n",
      "apps/two/lib/index.ts":
        "import { value } from '@/value'\nimport { shared } from '@example/shared/value'\nvoid [value, shared]\n",
      "packages/shared/package.json": JSON.stringify({
        name: "@example/shared",
        type: "module",
        exports: { "./value": { import: "./src/value.ts", require: "./src/required.cts" } },
      }),
      "packages/shared/src/value.ts": "export const shared = 3\n",
      "packages/shared/src/required.cts": "module.exports = { shared: 3 }\n",
      "apps/two/lib/common.cts": "const shared = require('@example/shared/value')\nvoid shared\n",
    };
    commit(root, files, "package aliases and exports");
    const archived = await buildGraph({ root, scope: "apps", staticTree: { gitRef: "fixture" } });
    expect(
      archived.edges.filter((edge) => edge.to === "packages/shared/src/value.ts"),
    ).toHaveLength(2);
    const packageLink = NodePath.join(root, "node_modules/@example/shared");
    NodeFS.mkdirSync(NodePath.dirname(packageLink), { recursive: true });
    NodeFS.symlinkSync(NodePath.join(root, "packages/shared"), packageLink, "dir");

    const graph = await buildGraph({ root, scope: "apps" });
    const targetsOf = (source: string) =>
      graph.edges
        .filter((edge) => edge.from === source)
        .map((edge) => edge.to)
        .sort();
    expect(targetsOf("apps/one/src/index.ts")).toEqual([
      "apps/one/src/value.ts",
      "packages/shared/src/value.ts",
    ]);
    expect(targetsOf("apps/two/lib/index.ts")).toEqual([
      "apps/two/lib/value.ts",
      "packages/shared/src/value.ts",
    ]);
    expect(targetsOf("apps/two/lib/common.cts")).toEqual(["packages/shared/src/required.cts"]);
    expect(graph.coverage).toMatchObject({
      supportedLanguages: ["javascript", "typescript"],
      analyzedFiles: 7,
      unresolvedLocalImports: { total: 22, omitted: 2 },
    });
    expect(graph.coverage?.unresolvedLocalImports.examples).toHaveLength(20);
    expect(
      graph.coverage?.unresolvedLocalImports.examples.every(
        (example) =>
          example.fileId === "apps/one/src/index.ts" && example.specifier.startsWith("./missing-"),
      ),
    ).toBe(true);
    expect(normalizeGraphJson(JSON.parse(JSON.stringify(graph))).coverage).toEqual(graph.coverage);
    const index = buildAtlasIndex(graph, graphContentDigest(JSON.stringify(graph)));
    expect(parseAtlasIndex(index).coverage).toEqual(graph.coverage);
    expect(index.units.systems.map((system) => system.label).sort()).toEqual([
      "One",
      "Shared",
      "Two",
    ]);
    expect(() =>
      parseAtlasIndex({
        ...
        coverage: {
          ...graph.coverage,
          unresolvedLocalImports: { total: 0, examples: [], omitted: 1 },
        },
      }),
...
describe("captured snapshot metadata", () => {
  it("keeps the captured repository ref in persisted artifacts inside another Git repository", async () => {
    const parent = gitRepo();
    commit(parent, { "parent.ts": "export const parent = true\n" }, "unrelated parent");
    const source = gitRepo();
    commit(source, { "src/source.ts": "export const source = true\n" }, "captured source");
    const capturedRef = NodeChildProcess.execFileSync("git", ["-C", source, "rev-parse", "HEAD"], {
      encoding: "utf8",
    }).trim();
    const parentRef = NodeChildProcess.execFileSync("git", ["-C", parent, "rev-parse", "HEAD"], {
      encoding: "utf8",
    }).trim();
    const capturedRoot = NodePath.join(parent, ".456code", "captured");
    const outDir = NodePath.join(parent, ".456code", "artifacts");
    NodeFS.mkdirSync(NodePath.join(capturedRoot, "src"), { recursive: true });
    NodeFS.copyFileSync(
      NodePath.join(source, "src/source.ts"),
      NodePath.join(capturedRoot, "src/source.ts"),
    );
    await runBuild(capturedRoot, {
      scope: ".",
      out: outDir,
      "no-history": true,
      "static-tree-ref": capturedRef,
    });
    const graph = normalizeGraphJson(
      JSON.parse(NodeFS.readFileSync(NodePath.join(outDir, "graph.json"), "utf8")),
    );
    const index = parseAtlasIndex(
      JSON.parse(NodeFS.readFileSync(NodePath.join(outDir, "atlas-index.json"), "utf8")),
    );
    expect(capturedRef).not.toBe(parentRef);
    expect(graph.gitRef).toBe(capturedRef);
    expect(index.repo.gitRef).toBe(capturedRef);
    expect(graph.repoRoot).toBe(".");
    expect(graph.nodes.map((node) => node.id)).toEqual(["src/source.ts"]);
    expect(graph.coChanges).toBeUndefined();
    expect(NodeFS.existsSync(NodePath.join(outDir, "graph.db"))).toBe(false);
  });
});

describe("namespace import provenance", () => {
  it("retains declared namespace names without claiming exact exported-name coverage", async () => {
    const root = tempDir();
    const files = {
      "api.ts": "export const value = 1\nexport interface Shape { value: number }\n",
      "consumer.ts":
        "import * as Api from './api.js'\nimport type * as ApiTypes from './api.js'\nimport { value } from './api.js'\nexport const values = [Api.value, value]\nexport type Shape = ApiTypes.Shape\n",
      "types.ts": "import type * as Types from './api.js'\nexport type Shape = Types.Shape\n",
      "side-effect.ts": "import './api.js'\n",
      "dynamic.ts": "export const load = () => import('./api.js')\n",
    };
    for (const [path, content] of Object.entries(files))
      NodeFS.writeFileSync(NodePath.join(root, path), content);
    const graph = await buildGraph({ root, scope: ".", staticTree: { gitRef: "fixture" } });
    const path = NodePath.join(root, "graph.json");
    NodeFS.writeFileSync(path, JSON.stringify(graph));
    const persisted = loadContextQuery(path).graph;
    const edge = (from: string) =>
      persisted.edges.find((entry) => entry.from === from && entry.to === "api.ts")!;
    expect(edge("consumer.ts").namespaceImports).toEqual([
      { name: "Api", typeOnly: false },
      { name: "ApiTypes", typeOnly: true },
    ]);
    expect(edge("consumer.ts").symbols).toBeUndefined();
    expect(edge("consumer.ts").typeOnly).toBeUndefined();
    expect(edge("types.ts").namespaceImports).toEqual([{ name: "Types", typeOnly: true }]);
    expect(edge("types.ts").typeOnly).toBe(true);
    expect(edge("side-effect.ts").symbols).toEqual([]);
    expect(edge("side-effect.ts").namespaceImports).toBeUndefined();
    expect(edge("dynamic.ts").symbols).toBeUndefined();
    expect(edge("dynamic.ts").namespaceImports).toBeUndefined();
    expect(edge("dynamic.ts").dynamic).toBe(true);
  });
});
