...
456code connects directly to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving your editor. Work stays in flow—no more jumping between browser tabs and terminal windows.

## Supported Providers

456code works with the platforms your team already uses:
...
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in 456code and verify GitHub shows as authenticated
...
1. Create an API token in your Atlassian account with read/write access to pull requests and
   repositories, plus read access to your user account (`read:user:bitbucket`, used to verify the
   connection)
2. Add these environment variables to the environment running 456code:
   ```bash
   export T3CODE_BITBUCKET_EMAIL="you@example.com"
   export T3CODE_BITBUCKET_API_TOKEN="your-token"
   ```
3. Restart 456code and verify the connection in **Source Control settings**
...
**Git is required** – 456code uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running 456code (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.
