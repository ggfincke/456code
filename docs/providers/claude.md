...
This guide is for people who want to use more than one Claude setup in 456code.
...
In 456code Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means 456code uses your normal home directory.
...
Then add another Claude provider in 456code:
...
456code only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so 456code keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in 456code Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. 456code stores the value as a server secret and does not
...
456code does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in 456code.
